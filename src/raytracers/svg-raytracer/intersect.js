 ...
 * @returns {HitInfo} - 
 */
function intersectLineSegment(ray, x1, y1, x2, y2){
 ...
 * @returns {HitInfo} - 
 */
function intersectTriangle(ray, cx, cy, angle, size){
    ...
    // intersect each side
    const hitA = intersectLineSegment(ray, vertices[0].x, vertices[0].y, vertices[1].x, vertices[1].y);
    const hitB = intersectLineSegment(ray, vertices[1].x, vertices[1].y, vertices[2].x, vertices[2].y);
    const hitC = intersectLineSegment(ray, vertices[2].x, vertices[2].y, vertices[0].x, vertices[0].y);
 ...
 * @returns {HitInfo} - 
 */
function intersectSphericalLens(ray, cx, cy, angle, diameter, centerThickness, edgeThickness)
{
    let result = new HitInfo(9999, ray.x+ray.dx*9999, ray.y+ray.dy*9999, 0, 0, -1);
    ...
    // intersect circles
    const leftHit = intersectCircle(ray, leftCircle.cx, leftCircle.cy, leftCircle.r);
    const rightHit = intersectCircle(ray, rightCircle.cx, rightCircle.cy, rightCircle.r);
...
export {HitInfo, intersectCircle, intersectLineSegment, intersectTriangle, intersectSphericalLens, intersectRectangle}